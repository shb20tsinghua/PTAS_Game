...
class Game:
    ...
    def solve(self, policy, record_file, verbose=0, plots=False):
        """
        Parameters:
        ...
        while True:
            subnit, opt_ave, opt_red, adam_m, adam_v = 0, 0, 1, np.zeros((Nn, Ns, Ni, Na)), np.zeros((Nn, Ns, Ni, Na))
            while True:
                projgrad, policy_bias, regret_bias, regret, dp_res, piU_jac, cano_sect = self.onto_equilbundl(barr, policy, value)
                record_list = [norm(item, np.inf, axis=-1).max(axis=-1) for item in [cano_sect, policy_bias, regret_bias, projgrad]]+[dp_res.max(axis=1)-dp_res.min(axis=1), beta]
                ...
                elif (subnit := subnit+1) > Game.maxsubnit or (projgrad_norm < 1e-10).all() or (np.abs(opt_red) < 1e-20).all() or (norsteplen < 1e-12).all() and (value_resangle < 1e-10).all():
            ...
            policy = (lambda vec: vec/np.sum(vec, axis=-1, keepdims=True))(np.exp(np.log(policy)-np.einsum('nsiakl,nskl->nsia', diff, dbarr)))
    ...
    def onto_equilbundl(self, barr, policy, value):
        ...
        piU_jac = (lambda _policy: np.block([[np.einsum(f"{Game.syms[:Ni]}sn{''.join([f',ns{k}' for k in Game.syms[:Ni].replace(Game.syms[i], '').replace(Game.syms[j], '')])}->ns{Game.syms[i]}{Game.syms[j]}", ustatic[..., i], *_policy[(np.arange(Ni) != i) & (np.arange(Ni) != j)]) if j != i else np.zeros((Nn, Ns, Na, Na)) for j in range(Ni)] for i in range(Ni)]))(policy.swapaxes(1, 2).swapaxes(0, 1))
        ...
        value_static = np.array([[[root_scalar(lambda v: (barr[n, s, i]/(v+_cano_sect[n, s, i])).sum()-1, bracket=np.array([0, 1000]), method='brentq').root for i in range(Ni)] for s in range(Ns)] for n in range(Nn)])+_piU_vec_max
