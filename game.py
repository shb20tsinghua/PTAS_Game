...
class Game:
    ...
    maxnit, maxsubnit, gTOL, bTOL, sglTOL, pdbiasTOL, tansumnTOL, resol = 1000000, 10000, 1e-6, 1e-5, 0.9, 1e-3, 1e-2, 1e-10*(1-1e-6)
    ...
    def solve(self, barr, policy, value):
        ...
        Norm_tansum_last, Norm_tan_last = np.inf, np.inf
        while True:
            ...
            while True:
                ...
                pd_unbiased, pg_zero, vres_opt, opt_monit = ((policy_biasnorm < Game.gTOL) & (regret_biasnorm < Game.gTOL)).all(), (projgrad_norm < 1e-10).all(), (value_resangle < 1e-10).all(), np.array([regret_biasnorm.max(), projgrad_norm.max(), value_resangle.max()])
                opt_ave, opt_red, opt_ocil = (lambda _red: (opt_monit+0.9*_red, _red, opt_red*_red))(opt_ave-opt_monit)
                ...
                if (nit := nit+1) > Game.maxnit:
                    ...
                    return (barr, policy, value), False, nit, [barr, dbarr, Ori_tan, Collin_tan, pb_bias, Norm_tansum, Norm_tan, policy_biasnorm, regret_biasnorm, projgrad_norm, value_resangle, norsteplen]
                    ...
                    if pd_unbiased:
                        if (barr < Game.bTOL).all() or ((pb_bias := norm(barr-policy*np.sum(barr, axis=-1, keepdims=True), np.inf, axis=-1)) < Game.bTOL).all():
                            np.savetxt(logrecord, np.vstack(self.record), delimiter=',')
                            return (barr, policy, value), True, nit, [barr, dbarr, Ori_tan, Collin_tan, pb_bias, Norm_tansum, Norm_tan, policy_biasnorm, regret_biasnorm, projgrad_norm, value_resangle, norsteplen]
                    else:
            ...
            Norm_tansum = norm(tanvec.sum(axis=-1), axis=1)
            ...
            conv_encounter = (Norm_tansum < Norm_tansum_last).all(axis=-1) & (Norm_tansum < Game.tansumnTOL).all(axis=-1) & (pb_bias < Game.pdbiasTOL).all(axis=-1)
            Norm_tan_last, Norm_tansum_last = Norm_tan, Norm_tansum

            if bback := (sgl_encounter.any() or (tansteplen < Game.maxtansteplen*1e0) or conv_encounter.all()):
                barr += policy*Game.backward_steplen
                exp_tanvec, tanvec = self.tanvec(barr, policy, sigmaU_jac)
                tansteplen = Game.maxtansteplen

            bpv_bkp = barr.copy(), policy.copy(), value.copy(), sigmaU_jac.copy()
            # recstr = f"|{nit:^7d}|{repr(bback):5}|{'|'.join([arr2str(item) for item in [barr, Ori_tan, Collin_tan, pb_bias, Norm_tansum, Norm_tan, policy_biasnorm, regret_biasnorm, projgrad_norm, value_resangle, norsteplen, tansteplen]])}|\n"
            # with open(frecord, 'a') as fio:
            #     fio.writelines(recstr)
            ...
            barr_next = np.where((barr < Game.bTOL).all(axis=(-1, -2))[:, np.newaxis, np.newaxis], barr, (barr*(1-(tansteplen/norm(tanvec.sum(axis=(-1, -2)), axis=-1).max()).clip(max=Game.maxdbarr*tansteplen/Game.maxtansteplen))).clip(min=Game.resol))
    ...
    def projgrad(self, barr, policy, value):
        ...
        projgrad = (lambda grad: grad-(grad.sum(axis=-1)/Na)[..., np.newaxis])(regret_bias-np.einsum('sab,sa->sb', sigmaU_jac, policy_bias.reshape((Ns, Ni*Na))).reshape((Ns, Ni, Na)))
